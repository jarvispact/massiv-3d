...
export const createRenderSystem = (gl: WebGL2RenderingContext, world: World, args: CreateRenderSystemArgs) => {
    ...
    const createCachedEntity = (entity: Entity) => {
        ...
        const colorBuffer = createWebgl2ArrayBuffer(gl, Float32Array.from(geometry.data.normals));
        ...
        gl.uniform3fv(colorLocation, material.data.color);
    ...
    return () => {
        const entities = world.queryEntities(['Transform', 'Geometry', 'Material']);
